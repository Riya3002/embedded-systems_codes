...
void loop() {
      ...
      if (client.available()) {  // if there's bytes to read from the client,
        ...
        if (c == '\n') {  // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("#button::before { content: "
                           "; pointer-events: none; position: absolute; background: #04da97; top: 110%; left: 0; width: 100%; height: 100%; transform: perspective(1em) rotateX(40deg) scale(1, 0.45); filter: blur(1em); opacity: 0.4;}");
            client.println("#button::after { content: "
                           "; position: absolute; top: 0; left: 0; bottom: 0; right: 0; box-shadow: 0 0 2em 0.5em #04da97; opacity: 0; transition: opacity 100ms linear;}");
            ...
            client.println("#button2::before { pointer-events: none; content: "
                           "; position: absolute; background: #fc5c34; top: 110%; left: 0; width: 100%; height: 100%; transform: perspective(1em) rotateX(40deg) scale(1, 0.45); filter: blur(1em); opacity: 0.4;}");
            client.println("#button2:hover, #button2:focus { background-color: #fc5c34; color: #333; text-shadow: none;}");
            client.println("#button2::after { content: "
                           "; position: absolute; top: 0; left: 0; bottom: 0; right: 0; box-shadow: 0 0 2em 0.5em #fc5c34; opacity: 0; transition: opacity 100ms linear;}");
  ...
  if (lfr == "on") {
    // Reading IR sensors
    int c = digitalRead(ir1);
    int d = digitalRead(ir2);
